...
public class MessageHelper {
	...
	public static String createChat(String accessToken, ChatInfo chatInfo)
			throws Exception {
		ChatService chatService = ServiceFactory.getInstance().getOpenService(ChatService.class);
		return  chatService.createChat(accessToken, chatInfo.name,
				chatInfo.owner, chatInfo.useridlist);
	}
	public static com.dingtalk.open.client.api.model.corp.ChatInfo getChat(String accessToken, String chatid)
			throws Exception {
		ChatService chatService = ServiceFactory.getInstance().getOpenService(ChatService.class);
		return  chatService.getChat(accessToken, chatid);
	}
